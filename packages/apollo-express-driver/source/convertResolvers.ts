   ...
   for (const [typeName, fields] of entries(_.res)) {
      ...
      for (const [fieldName, resolver] of entries(fields)) {
         if (typeof resolver === "function") {
            (resolvers as any)[fieldName] = (root: any, args: any, ctx: RequestContext) =>
               pipe(
                  (resolver as any)({
                     args: args || {},
                     ctx,
            ...
            (resolvers as any)[fieldName] = {
               subscribe: (root: any, args: any, ctx: RequestContext) =>
                  pipe(
            ...
            if ((resolver as any).resolve) {
               (resolvers as any)[fieldName].resolve = (x: any, _: any, ctx: RequestContext) =>
                  pipe(
